...
func newLinodeCloudProvider(manager *Manager, rl *cloudprovider.ResourceLimiter) (*linodeCloudProvider, error) {
...
func (l *linodeCloudProvider) NodeGroups() []cloudprovider.NodeGroup {
	...
	for i, ng := range l.manager.nodeGroups {
		nodeGroups[i] = ng
	}
	return nodeGroups
}
...
func (l *linodeCloudProvider) NodeGroupForNode(*apiv1.Node) (cloudprovider.NodeGroup, error) {
	//TODO bonf bisogna capire come mappare apiv1.Node a un nodo su linode, poi è facile tornare a node group
}
...
func BuildLinode(
	...
	if err != nil {
		klog.Fatalf("Failed to create Linode manager: %v", err)
	}

	provider, err := newLinodeCloudProvider(manager, rl)
	if err != nil {
		klog.Fatalf("Failed to create Linode cloud provider: %v", err)
	}
